    ...
    internal class LoaderWrapper
    {
        ...
        internal string Timezone
        {
            ...
            set
            {
                ...
                if (false == Enumerable.Any(TimeZoneInfo.GetSystemTimeZones(), item => item.Id == value))
                    throw new Exception(string.Format("Timezone \"{value}\" not found in your system."));
        ...
        private _REG_TZI_FORMAT ReadTZIFromRegistry(string id)
        {
            var tzi =
                (byte[])
                Registry.GetValue(
                                  string.Format("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Time Zones\\{id}"),
                                  "TZI",
                                  null);
        ...
        internal struct RTL_TIME_ZONE_INFORMATION
        {
            ...
            {
                return string.Format("StandardName={Encoding.Unicode.GetString(StandardName)}");
            }
